...
fileInput.addEventListener('change', () => {
  ...
  if (file) document.querySelector('.file-drop').innerHTML = `<strong>${file.name}</strong><small>${(file.size / 1024).toFixed(1)} KB · Ready to analyze</small>`;
});
...
function analyze() {
  ...
  document.querySelector('#answerHeading').textContent = question.endsWith('?') ? `Here’s the clearest answer.` : `Here’s what stands out.`;
  document.querySelector('#answerText').textContent = hasFile
    ? `I’m ready to review “${fileInput.files[0].name}” for ${question.toLowerCase()}. This demo response shows where your connected model’s analysis will appear.`
    : `The text suggests a clear focus on making complex information easier to understand. For “${question.toLowerCase()}”, the strongest signals are clarity, confidence, and keeping the reader moving toward a decision.`;
...
textInput.addEventListener('keydown', (event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); analyze(); } });
clearButton.addEventListener('click', () => {
  questionInput.value = ''; textInput.value = ''; fileInput.value = ''; characterCount.textContent = '0'; emptyState.hidden = false; answerState.hidden = true;
  confidenceValue.textContent = '—'; confidenceMeter.style.width = '0'; confidenceNote.textContent = 'Waiting for a question'; confidenceLabel.textContent = 'No analysis yet'; document.querySelector('.pulse').classList.remove('active');
});
document.addEventListener('keydown', (event) => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); clearButton.click(); } });

document.querySelector('#settingsButton').addEventListener('click', () => { settingsModal.hidden = false; document.querySelector('#apiKeyInput').focus(); });
document.querySelector('#closeModal').addEventListener('click', () => { settingsModal.hidden = true; });
